...
class TestEdgeBotStandalone:
    ...
    def create_edgebot_config(self) -> Path:
        ...
        config = {
            "server": {"host": "127.0.0.1", "port": self.main_port},
            "inputs": {
                "syslog": {
                    "enabled": True,
            ...
            "observability": {"health_port": self.health_port, "metrics_enabled": True},
        }
    ...
    async def test_edgebot_basic_functionality(self):
        ...
        ), f"EdgeBot not healthy: {health_data}"

        # Send test syslog messages
        test_messages = SyslogSender.send_test_messages(
            count=3, host="localhost", port=self.syslog_udp_port
        )
            ...
            with open(output_file, "r") as f:
                for line in f:
                    if line.strip():  # Skip empty lines
                        ...
                        assert (
                            "hostname" in event
                        ), f"Event missing 'hostname' field: {event}"

                        # Check for test message content
                        if "edgebot-test" in event.get("message", ""):
                            assert "testhost" in event.get(
                                "hostname", ""
                            ), "Test hostname not preserved"
    ...
    async def test_edgebot_metrics_endpoint(self):
        ...
        test_messages = SyslogSender.send_test_messages(
            count=5, host="localhost", port=self.syslog_udp_port
        )
        ...
        for line in metrics_text.split("\n"):
            if line.startswith("edgebot_messages_received_total"):
                ...
                try:
                    ...
                    assert (
                        value > 0
                    ), f"Message received counter should be > 0, got {value}"
    ...
    async def test_edgebot_multiple_message_formats(self):
        ...
        assert len(events_found) >= len(
            test_messages
        ), f"Expected >= {len(test_messages)} events, found {len(events_found)}"

        # Check that different message types were processed
        message_contents = [event.get("message", "") for event in events_found]
        assert any(
            "RFC3164" in msg for msg in message_contents
        ), "RFC3164 message not found"
        assert any(
            "RFC5424" in msg for msg in message_contents
        ), "RFC5424 message not found"
        assert any(
            "System test message" in msg for msg in message_contents
    ...
    async def test_edgebot_configuration_validation(self):
        ...
        success = self.process_manager.start_process(
            "edgebot-invalid", cmd, cwd=str(edgebot_dir), env=env
        )
