...
from tests.e2e.utils import (
    DockerComposeManager, ProcessManager, SyslogSender, HealthChecker,
    LokiClient, TestDataManager, find_free_port
)
...
class TestCrossPhaseIntegration:
    ...
    async def test_full_integration_pipeline(self):
        ...
        assert "edgebot_messages_received_total" in metrics_text, "EdgeBot receive metrics missing"
        assert "edgebot_output_shipped_total" in metrics_text, "EdgeBot ship metrics missing"
        ...
        assert total_events >= 5, f"Expected at least 5 events processed by Mothership, got {total_events}"
        ...
        if loki_result.get('status') == 'success':
            ...
            assert matching_entries >= 3, f"Expected at least 3 matching entries in Loki, found {matching_entries}"
        else:
    ...
    async def test_integration_with_different_syslog_formats(self):
        ...
        assert total_events >= len(test_messages), f"Expected at least {len(test_messages)} events, got {total_events}"
    ...
    async def test_integration_error_recovery(self):
        ...
        metrics_text = response.text
        assert "edgebot_output_shipped_total" in metrics_text, "No shipped metrics found"
        ...
        assert total_events >= 3, f"Expected at least 3 events after recovery, got {total_events}"
    ...
    async def test_integration_metrics_end_to_end(self):
        ...
        for i in range(message_count):
            message = f"<34>Aug 23 12:05:45 metrics-host metrics-app: Metrics test #{i+1}"
            SyslogSender.send_message(message, host="localhost", port=self.syslog_udp_port)
        ...
        assert received_count >= message_count, f"EdgeBot received count {received_count} < {message_count}"
        assert shipped_count > 0, f"EdgeBot shipped count should be > 0, got {shipped_count}"
        ...
        assert "mothership_ingestion_events_total" in mothership_metrics, "Missing Mothership ingestion metrics"
        assert "mothership_pipeline_duration_seconds" in mothership_metrics, "Missing Mothership pipeline metrics"
        ...
        assert pipeline_stats.get('total_events', 0) >= message_count, "Not all events processed by pipeline"
        assert pipeline_stats.get('successful_events', 0) > 0, "No successful events in pipeline"