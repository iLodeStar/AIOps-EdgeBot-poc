...
class SyslogSender:
    ...
    def send_test_messages(
        cls, count: int = 5, host: str = "localhost", port: int = 5514
    ) -> List[Dict]:
...
class LokiClient:
    ...
    def query(self, query_string: str, start_time: Optional[datetime] = None) -> Dict:
        ...
        if start_time:
            params["start"] = str(
                int(start_time.timestamp() * 1000000000)
            )  # nanoseconds
...
class TestDataManager:
    ...
    def create_edgebot_config(output_dir: Path, mothership_url: str = None) -> Path:
        ...
        config = {
            ...
            "inputs": {"syslog": {"enabled": True, "udp_port": 5514, "tcp_port": 5515}},
            "output": {},
            "observability": {"health_port": 8081, "metrics_enabled": True},
        }
    ...
    @staticmethod
    def create_mothership_config(
        temp_dir: Path, loki_enabled: bool = True, tsdb_enabled: bool = False
    ) -> Path:
        """Create a test Mothership configuration file."""
        config = {
            "server": {"host": "0.0.0.0", "port": 8443},
            "sinks": {
                "loki": {"enabled": loki_enabled, "url": "http://localhost:3100"},
                "tsdb": {"enabled": tsdb_enabled},
            },
            "pipeline": {
                "processors": [
                    {"type": "drop_fields", "config": {"fields": ["_internal"]}},
                    {
                        "type": "add_tags",
...
def wait_for_file_content(
    file_path: Path, expected_lines: int = 1, timeout: int = 30
) -> bool:
