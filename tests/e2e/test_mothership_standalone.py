...
from tests.e2e.utils import (
    DockerComposeManager, ProcessManager, HealthChecker, 
...
class TestMothershipStandalone:
    ...
    async def test_mothership_basic_ingestion(self):
        ...
        assert health_data.get('status') == 'healthy', f"Mothership not healthy: {health_data}"
    ...
    async def test_mothership_pipeline_processing(self):
        ...
        assert pipeline_stats.get('total_events', 0) > 0, f"No events processed: {stats_data}"
        
        # Check processor stats
        processors = pipeline_stats.get('processors', {})
        assert 'DropFields' in processors, f"DropFields processor not found: {processors}"
    ...
    async def test_mothership_loki_integration(self):
        ...
        if loki_result.get('status') == 'success':
            ...
            assert found_message, f"Test message with ID {test_id} not found in Loki results"
        else:
            ...
            stats_data = response.json()
            pipeline_stats = stats_data.get('pipeline', {})
            assert pipeline_stats.get('total_events', 0) > 0, "No events processed by pipeline"
    ...
    async def test_mothership_error_handling(self):
        ...
        assert response.status_code in [200, 400], f"Unexpected response for invalid events: {response.status_code}"
        
        # Test 3: Verify service is still healthy after errors
        health_url = f"http://localhost:{self.mothership_port}/healthz"
        response = requests.get(health_url, timeout=10)
        assert response.status_code == 200, "Service became unhealthy after error handling"
    ...
    async def test_mothership_batch_processing(self):
        ...
        assert total_events >= batch_size, f"Expected at least {batch_size} events processed, got {total_events}"