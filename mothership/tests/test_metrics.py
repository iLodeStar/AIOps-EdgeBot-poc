...
from mothership.app.metrics import (
    get_metrics_content,
    mship_ingest_batches_total, mship_ingest_events_total, mship_written_events_total,
    mship_sink_written_total, mship_ingest_seconds, mship_pipeline_seconds,
    mship_sink_write_seconds, mship_requests_total, mship_active_connections,
    mship_loki_queue_size, METRICS_REGISTRY
)
...
class TestMetrics:
    ...
    def test_per_sink_metrics(self):
        ...
        mship_requests_total.labels(method='GET', endpoint='/metrics', status='200').inc()
        ...
        assert 'mship_requests_total{endpoint="/ingest",method="POST",status="200"}' in metrics_content
    ...
    def test_custom_registry_isolation(self):
        ...
        default_counter = Counter('test_default_counter', 'Test counter in default registry')
        default_counter.inc()
...
class TestMetricsRequirements:
    ...
    def test_required_counters_exist(self):
        ...
        for counter_name in required_counters:
            assert counter_name in content, f"Required counter {counter_name} not found in metrics"
    ...
    def test_required_histograms_exist(self):
        ...
        for histogram_name in required_histograms:
            assert histogram_name in content, f"Required histogram {histogram_name} not found in metrics"
