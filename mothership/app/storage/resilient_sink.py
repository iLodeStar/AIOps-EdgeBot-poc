...
from .reliability import (
    SinkRetryManager, SinkCircuitBreaker, SinkPersistentQueue,
...
class ResilientSink:
    ...
    def __init__(self, name: str, wrapped_sink: StorageSink, config: Dict[str, Any]):
        ...
        self.retry_manager = SinkRetryManager(name, retry_config) if retry_config.get('enabled', True) else None
        self.circuit_breaker = SinkCircuitBreaker(name, circuit_config) if circuit_config.get('enabled', True) else None
        self.persistent_queue = SinkPersistentQueue(name, queue_config) if queue_config.get('enabled', False) else None
        ...
        try:
            ...
            if self.retry_manager:
                result = await self.retry_manager.execute_with_retry(
                    lambda: self.wrapped_sink.write_events(events),
                    events
                )
            else:
            ...
            logger.error("Non-retryable error, not queuing", sink=self.name, error=str(e))
            return {"written": 0, "errors": len(events), "retries": 0, "queued": 0}
            ...
            if self.persistent_queue:
                logger.warning("Direct write failed, queuing events", sink=self.name, error=str(e))
                queued = await self._queue_events(events)
                return {"written": 0, "errors": 0, "retries": 0, "queued": queued}
            else:
                logger.error("Direct write failed, no queue available", sink=self.name, error=str(e))
                return {"written": 0, "errors": len(events), "retries": 0, "queued": 0}
    ...
    async def _process_queue(self):
        ...
        while not self._shutdown:
            try:
                ...
                logger.debug("Processing queued events", sink=self.name, count=len(batch))
                ...
                try:
                    ...
                    if self.retry_manager:
                        result = await self.retry_manager.execute_with_retry(
                            lambda: self.wrapped_sink.write_events(batch),
                            batch
                        )
                    else:
                    ...
                    logger.debug("Successfully processed queued events", 
                               sink=self.name, count=len(batch))
                    
                except NonRetryableException as e:
                    # Non-retryable error - nack events (may go to DLQ)
                    if self.circuit_breaker:
                        await self.circuit_breaker.record_failure()
                    await self.persistent_queue.nack_events(batch, max_retries=0)  # Immediate DLQ
                    logger.error("Non-retryable error processing queue", 
                               sink=self.name, error=str(e))
                    ...
                    logger.warning("Error processing queued events, will retry", 
                                 sink=self.name, error=str(e))
                
            except Exception as e:
                logger.error("Unexpected error in queue processor", 
                           sink=self.name, error=str(e))
                await asyncio.sleep(flush_interval)
