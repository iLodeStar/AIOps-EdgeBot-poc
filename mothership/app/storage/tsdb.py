...
class TimescaleDBWriter:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info("Initialized TimescaleDB writer",
                   dsn=self._safe_dsn(), 
    ...
    async def insert_events(self, events: List[Dict[str, Any]]) -> bool:
        ...
        try:
            async with self.pool.connection() as conn:
                ...
                logger.info("Inserted events batch", 
                           count=len(events), 
                           table=self.table_name)
                ...
                if isinstance(ts_value, str):
                    ...
                    try:
                        if ts_value.endswith('Z'):
                            return datetime.fromisoformat(ts_value[:-1]).replace(tzinfo=timezone.utc)
    ...
    def _extract_source(self, event: Dict[str, Any]) -> str:
        ...
        source_fields = ['source', 'host', 'hostname', 'source_host', 'client_ip', 'source_ip']
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.pool.connection() as conn:
                async with conn.cursor() as cursor:
                    # Count total events
                    await cursor.execute(f"SELECT COUNT(*) as count FROM {self.table_name}")
