...
class LokiConfig(BaseModel):
    ...
    def __init__(self, **data):
        ...
        if "enabled" not in data:
            data["enabled"] = os.getenv("LOKI_ENABLED", "false").lower() in (
                "true",
                "1",
                "yes",
                "on",
        ...
        if "batch_timeout_seconds" not in data:
            data["batch_timeout_seconds"] = float(
                os.getenv("LOKI_BATCH_TIMEOUT_SECONDS", "5.0")
            )
        if "max_retries" not in data:
            data["max_retries"] = int(os.getenv("LOKI_MAX_RETRIES", "3"))
        if "retry_backoff_seconds" not in data:
            data["retry_backoff_seconds"] = float(
                os.getenv("LOKI_RETRY_BACKOFF_SECONDS", "1.0")
            )
...
class TSDBConfig(BaseModel):
    ...
    def __init__(self, **data):
        ...
        # Set defaults from environment variables if not explicitly provided
        if "enabled" not in data:
            data["enabled"] = os.getenv("TSDB_ENABLED", "true").lower() in (
                "true",
                "1",
                "yes",
                "on",
...
class AppConfig(BaseModel):
    ...
    def from_env(cls) -> "AppConfig":
        ...
        return cls(loki=LokiConfig(), tsdb=TSDBConfig())
...
class ConfigManager:
    ...
    def _apply_env_overrides(self):
        ...
        if os.getenv("MOTHERSHIP_PORT"):
            self._config.setdefault("server", {})["port"] = int(
                os.getenv("MOTHERSHIP_PORT")
            )

        # Database configuration
        if os.getenv("MOTHERSHIP_DB_DSN"):
            self._config.setdefault("database", {})["dsn"] = os.getenv(
                "MOTHERSHIP_DB_DSN"
            )
        if os.getenv("MOTHERSHIP_DB_HOST"):
            self._config.setdefault("database", {})["host"] = os.getenv(
                "MOTHERSHIP_DB_HOST"
            )
        if os.getenv("MOTHERSHIP_DB_PORT"):
            self._config.setdefault("database", {})["port"] = int(
                os.getenv("MOTHERSHIP_DB_PORT")
            )
        if os.getenv("MOTHERSHIP_DB_NAME"):
            self._config.setdefault("database", {})["database"] = os.getenv(
                "MOTHERSHIP_DB_NAME"
            )
        if os.getenv("MOTHERSHIP_DB_USER"):
            self._config.setdefault("database", {})["user"] = os.getenv(
                "MOTHERSHIP_DB_USER"
            )
        if os.getenv("MOTHERSHIP_DB_PASS"):
            self._config.setdefault("database", {})["password"] = os.getenv(
                "MOTHERSHIP_DB_PASS"
            )

        # LLM configuration
        if os.getenv("MOTHERSHIP_LLM_ENABLED"):
            self._config.setdefault("llm", {})["enabled"] = os.getenv(
                "MOTHERSHIP_LLM_ENABLED"
            ).lower() in ("true", "1", "yes", "on")
        if os.getenv("MOTHERSHIP_LLM_ENDPOINT"):
            self._config.setdefault("llm", {})["endpoint"] = os.getenv(
                "MOTHERSHIP_LLM_ENDPOINT"
            )
        if os.getenv("MOTHERSHIP_LLM_API_KEY"):
            self._config.setdefault("llm", {})["api_key"] = os.getenv(
                "MOTHERSHIP_LLM_API_KEY"
            )
        if os.getenv("MOTHERSHIP_LLM_MODEL"):
            self._config.setdefault("llm", {})["model"] = os.getenv(
                "MOTHERSHIP_LLM_MODEL"
            )
        if os.getenv("MOTHERSHIP_LLM_CONFIDENCE_THRESHOLD"):
            self._config.setdefault("llm", {})["confidence_threshold"] = float(
                os.getenv("MOTHERSHIP_LLM_CONFIDENCE_THRESHOLD")
            )
        ...
        if os.getenv("OLLAMA_BASE_URL"):
            self._config.setdefault("llm", {})["ollama_base_url"] = os.getenv(
                "OLLAMA_BASE_URL"
            )
        if os.getenv("OLLAMA_MODEL"):
            self._config.setdefault("llm", {})["ollama_model"] = os.getenv(
                "OLLAMA_MODEL"
            )
        if os.getenv("OLLAMA_TIMEOUT_MS"):
            self._config.setdefault("llm", {})["ollama_timeout_ms"] = int(
                os.getenv("OLLAMA_TIMEOUT_MS")
            )
        if os.getenv("OLLAMA_MAX_TOKENS"):
            self._config.setdefault("llm", {})["ollama_max_tokens"] = int(
                os.getenv("OLLAMA_MAX_TOKENS")
            )

        # Loki sink configuration - NEW
        if os.getenv("LOKI_ENABLED"):
            self._config.setdefault("sinks", {}).setdefault("loki", {})["enabled"] = (
                os.getenv("LOKI_ENABLED").lower() in ("true", "1", "yes", "on")
            )
        if os.getenv("LOKI_URL"):
            self._config.setdefault("sinks", {}).setdefault("loki", {})["url"] = (
                os.getenv("LOKI_URL")
            )
        if os.getenv("LOKI_TENANT_ID"):
            self._config.setdefault("sinks", {}).setdefault("loki", {})["tenant_id"] = (
                os.getenv("LOKI_TENANT_ID")
            )
        if os.getenv("LOKI_USERNAME"):
            self._config.setdefault("sinks", {}).setdefault("loki", {})["username"] = (
                os.getenv("LOKI_USERNAME")
            )
        if os.getenv("LOKI_PASSWORD"):
            self._config.setdefault("sinks", {}).setdefault("loki", {})["password"] = (
                os.getenv("LOKI_PASSWORD")
            )
        if os.getenv("LOKI_BATCH_SIZE"):
            self._config.setdefault("sinks", {}).setdefault("loki", {})[
                "batch_size"
            ] = int(os.getenv("LOKI_BATCH_SIZE"))
        if os.getenv("LOKI_BATCH_TIMEOUT_SECONDS"):
            self._config.setdefault("sinks", {}).setdefault("loki", {})[
                "batch_timeout_seconds"
            ] = float(os.getenv("LOKI_BATCH_TIMEOUT_SECONDS"))

        # Loki reliability configuration
        if os.getenv("LOKI_MAX_RETRIES"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "retry", {}
            )["max_retries"] = int(os.getenv("LOKI_MAX_RETRIES"))
        if os.getenv("LOKI_INITIAL_BACKOFF_MS"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "retry", {}
            )["initial_backoff_ms"] = int(os.getenv("LOKI_INITIAL_BACKOFF_MS"))
        if os.getenv("LOKI_MAX_BACKOFF_MS"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "retry", {}
            )["max_backoff_ms"] = int(os.getenv("LOKI_MAX_BACKOFF_MS"))
        if os.getenv("LOKI_JITTER_FACTOR"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "retry", {}
            )["jitter_factor"] = float(os.getenv("LOKI_JITTER_FACTOR"))
        if os.getenv("LOKI_TIMEOUT_MS"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "retry", {}
            )["timeout_ms"] = int(os.getenv("LOKI_TIMEOUT_MS"))
        if os.getenv("LOKI_FAILURE_THRESHOLD"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "circuit_breaker", {}
            )["failure_threshold"] = int(os.getenv("LOKI_FAILURE_THRESHOLD"))
        if os.getenv("LOKI_OPEN_DURATION_SEC"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "circuit_breaker", {}
            )["open_duration_sec"] = int(os.getenv("LOKI_OPEN_DURATION_SEC"))
        if os.getenv("LOKI_HALF_OPEN_MAX_INFLIGHT"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "circuit_breaker", {}
            )["half_open_max_inflight"] = int(os.getenv("LOKI_HALF_OPEN_MAX_INFLIGHT"))
        if os.getenv("LOKI_QUEUE_ENABLED"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "queue", {}
            )["enabled"] = os.getenv("LOKI_QUEUE_ENABLED").lower() in (
                "true",
                "1",
                "yes",
                "on",
        ...
        if os.getenv("LOKI_QUEUE_DIR"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "queue", {}
            )["queue_dir"] = os.getenv("LOKI_QUEUE_DIR")
        if os.getenv("LOKI_QUEUE_MAX_BYTES"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "queue", {}
            )["queue_max_bytes"] = int(os.getenv("LOKI_QUEUE_MAX_BYTES"))
        if os.getenv("LOKI_QUEUE_FLUSH_INTERVAL_MS"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "queue", {}
            )["queue_flush_interval_ms"] = int(
                os.getenv("LOKI_QUEUE_FLUSH_INTERVAL_MS")
            )
        if os.getenv("LOKI_DLQ_DIR"):
            self._config.setdefault("sinks", {}).setdefault("loki", {}).setdefault(
                "queue", {}
            )["dlq_dir"] = os.getenv("LOKI_DLQ_DIR")

        # TimescaleDB sink configuration - NEW
        if os.getenv("TSDB_ENABLED"):
            self._config.setdefault("sinks", {}).setdefault("timescaledb", {})[
                "enabled"
            ] = os.getenv("TSDB_ENABLED").lower() in ("true", "1", "yes", "on")

        # TSDB reliability configuration
        if os.getenv("TSDB_MAX_RETRIES"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("retry", {})["max_retries"] = int(
                os.getenv("TSDB_MAX_RETRIES")
            )
        if os.getenv("TSDB_INITIAL_BACKOFF_MS"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("retry", {})["initial_backoff_ms"] = int(
                os.getenv("TSDB_INITIAL_BACKOFF_MS")
            )
        if os.getenv("TSDB_MAX_BACKOFF_MS"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("retry", {})["max_backoff_ms"] = int(
                os.getenv("TSDB_MAX_BACKOFF_MS")
            )
        if os.getenv("TSDB_JITTER_FACTOR"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("retry", {})["jitter_factor"] = float(
                os.getenv("TSDB_JITTER_FACTOR")
            )
        if os.getenv("TSDB_TIMEOUT_MS"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("retry", {})["timeout_ms"] = int(os.getenv("TSDB_TIMEOUT_MS"))
        if os.getenv("TSDB_FAILURE_THRESHOLD"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("circuit_breaker", {})["failure_threshold"] = int(
                os.getenv("TSDB_FAILURE_THRESHOLD")
            )
        if os.getenv("TSDB_OPEN_DURATION_SEC"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("circuit_breaker", {})["open_duration_sec"] = int(
                os.getenv("TSDB_OPEN_DURATION_SEC")
            )
        if os.getenv("TSDB_HALF_OPEN_MAX_INFLIGHT"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("circuit_breaker", {})["half_open_max_inflight"] = int(
                os.getenv("TSDB_HALF_OPEN_MAX_INFLIGHT")
            )
        if os.getenv("TSDB_QUEUE_ENABLED"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("queue", {})["enabled"] = os.getenv(
                "TSDB_QUEUE_ENABLED"
            ).lower() in (
                "true",
                "1",
        ...
        if os.getenv("TSDB_QUEUE_DIR"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("queue", {})["queue_dir"] = os.getenv("TSDB_QUEUE_DIR")
        if os.getenv("TSDB_QUEUE_MAX_BYTES"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("queue", {})["queue_max_bytes"] = int(
                os.getenv("TSDB_QUEUE_MAX_BYTES")
            )
        if os.getenv("TSDB_QUEUE_FLUSH_INTERVAL_MS"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("queue", {})["queue_flush_interval_ms"] = int(
                os.getenv("TSDB_QUEUE_FLUSH_INTERVAL_MS")
            )
        if os.getenv("TSDB_DLQ_DIR"):
            self._config.setdefault("sinks", {}).setdefault(
                "timescaledb", {}
            ).setdefault("queue", {})["dlq_dir"] = os.getenv("TSDB_DLQ_DIR")

        # Sink reliability configuration (new SATCOM-friendly defaults)
        if os.getenv("SINK_DEFAULT_MAX_RETRIES"):
            self._config.setdefault("sink_defaults", {})["max_retries"] = int(
                os.getenv("SINK_DEFAULT_MAX_RETRIES")
            )
        if os.getenv("SINK_DEFAULT_INITIAL_BACKOFF_MS"):
            self._config.setdefault("sink_defaults", {})["initial_backoff_ms"] = int(
                os.getenv("SINK_DEFAULT_INITIAL_BACKOFF_MS")
            )
        if os.getenv("SINK_DEFAULT_MAX_BACKOFF_MS"):
            self._config.setdefault("sink_defaults", {})["max_backoff_ms"] = int(
                os.getenv("SINK_DEFAULT_MAX_BACKOFF_MS")
            )
        if os.getenv("SINK_DEFAULT_JITTER_FACTOR"):
            self._config.setdefault("sink_defaults", {})["jitter_factor"] = float(
                os.getenv("SINK_DEFAULT_JITTER_FACTOR")
            )
        if os.getenv("SINK_DEFAULT_TIMEOUT_MS"):
            self._config.setdefault("sink_defaults", {})["timeout_ms"] = int(
                os.getenv("SINK_DEFAULT_TIMEOUT_MS")
            )
        if os.getenv("SINK_DEFAULT_FAILURE_THRESHOLD"):
            self._config.setdefault("sink_defaults", {})["failure_threshold"] = int(
                os.getenv("SINK_DEFAULT_FAILURE_THRESHOLD")
            )
        if os.getenv("SINK_DEFAULT_OPEN_DURATION_SEC"):
            self._config.setdefault("sink_defaults", {})["open_duration_sec"] = int(
                os.getenv("SINK_DEFAULT_OPEN_DURATION_SEC")
            )
        if os.getenv("SINK_DEFAULT_HALF_OPEN_MAX_INFLIGHT"):
            self._config.setdefault("sink_defaults", {})["half_open_max_inflight"] = (
                int(os.getenv("SINK_DEFAULT_HALF_OPEN_MAX_INFLIGHT"))
            )

        # Idempotency configuration
        if os.getenv("IDEMPOTENCY_WINDOW_SEC"):
            self._config.setdefault("idempotency", {})["window_sec"] = int(
                os.getenv("IDEMPOTENCY_WINDOW_SEC")
            )
        ...
        # Retry configuration
        if os.getenv("SINK_DEFAULT_MAX_RETRIES"):
            reliability_config["max_retries"] = int(
                os.getenv("SINK_DEFAULT_MAX_RETRIES")
            )
        if os.getenv("SINK_DEFAULT_INITIAL_BACKOFF_MS"):
            reliability_config["initial_backoff_ms"] = int(
                os.getenv("SINK_DEFAULT_INITIAL_BACKOFF_MS")
            )
        if os.getenv("SINK_DEFAULT_MAX_BACKOFF_MS"):
            reliability_config["max_backoff_ms"] = int(
                os.getenv("SINK_DEFAULT_MAX_BACKOFF_MS")
            )
        if os.getenv("SINK_DEFAULT_JITTER_FACTOR"):
            reliability_config["jitter_factor"] = float(
                os.getenv("SINK_DEFAULT_JITTER_FACTOR")
            )
        if os.getenv("SINK_DEFAULT_TIMEOUT_MS"):
        ...
        if os.getenv("SINK_DEFAULT_FAILURE_THRESHOLD"):
            reliability_config["failure_threshold"] = int(
                os.getenv("SINK_DEFAULT_FAILURE_THRESHOLD")
            )
        if os.getenv("SINK_DEFAULT_OPEN_DURATION_SEC"):
            reliability_config["open_duration_sec"] = int(
                os.getenv("SINK_DEFAULT_OPEN_DURATION_SEC")
            )
        if os.getenv("SINK_DEFAULT_HALF_OPEN_MAX_INFLIGHT"):
            reliability_config["half_open_max_inflight"] = int(
                os.getenv("SINK_DEFAULT_HALF_OPEN_MAX_INFLIGHT")
            )

        # Persistent queue configuration
        if os.getenv("QUEUE_ENABLED"):
            reliability_config["queue_enabled"] = os.getenv(
                "QUEUE_ENABLED"
            ).lower() in ("true", "1", "yes", "on")
        ...
        if os.getenv("QUEUE_FLUSH_INTERVAL_MS"):
            reliability_config["queue_flush_interval_ms"] = int(
                os.getenv("QUEUE_FLUSH_INTERVAL_MS")
            )
        if os.getenv("DLQ_DIR"):
            reliability_config["dlq_dir"] = os.getenv("DLQ_DIR")
        if os.getenv("FLUSH_BANDWIDTH_BYTES_PER_SEC"):
            reliability_config["flush_bandwidth_bytes_per_sec"] = int(
                os.getenv("FLUSH_BANDWIDTH_BYTES_PER_SEC")
            )
        if os.getenv("IDEMPOTENCY_WINDOW_SEC"):
            reliability_config["idempotency_window_sec"] = int(
                os.getenv("IDEMPOTENCY_WINDOW_SEC")
            )
    ...
    def _validate_config(self):
        ...
        if "dsn" not in database and not all(
            key in database for key in ["host", "port", "database", "user"]
        ):
            raise ValueError(
                "Database section must contain 'dsn' or connection parameters (host, port, database, user)"
            )
        ...
        return {
            ...
            "pipeline": {
                "processors": {
                    ...
                    "enrichment": {
                        "enabled": True,
                        "add_tags": {"processed_by": "mothership", "version": "1.5"},
                        "severity_mapping": {
                            "emergency": 0,
                            "alert": 1,
                            "critical": 2,
            ...
            "sinks": {
                "timescaledb": {"enabled": True},  # Default enabled
                "loki": {
            ...
            "idempotency": {"window_sec": 3600},  # 1 hour deduplication window
