...
class LLMEnricher(Processor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        if self.backend not in ["openai", "ollama"]:
            raise ValueError(
                f"Invalid LLM backend: {self.backend}. Must be 'openai' or 'ollama'"
            )
        ...
        self.circuit_breaker_enabled = config.get("circuit_breaker", {}).get(
            "enabled", True
        )
        self.failure_threshold = config.get("circuit_breaker", {}).get(
            "failure_threshold", 5
        )
    ...
    async def process(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(event_str) > self.max_event_size:
                logger.warning(
                    "Event too large for LLM processing", size=len(event_str)
                )
                return event
    ...
    async def _call_openai_llm(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = await self.client.post(
                f"{self.endpoint}/chat/completions", json=request_data, headers=headers
            )
    ...
    async def _call_ollama_llm(self, prompt: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            response = await self.client.post(
                f"{self.base_url}/api/chat", json=request_data, headers=headers
            )
            response.raise_for_status()
            ...
            if "message" in result and "content" in result["message"]:
                ...
                try:
                    ...
                    if json_match:
                        ...
                        logger.warning(
                            "No valid JSON found in Ollama response", content=content
                        )
                        return None
...
"""
    ...
    def _validate_response(self, response: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if "summary" in response:
                if (
                    not isinstance(response["summary"], str)
                    or len(response["summary"]) > 200
                ):
                    return False
    ...
    def _apply_llm_enrichment(
        self, event: Dict[str, Any], llm_response: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _record_failure(self):
        ...
        if self.circuit_breaker_enabled:
            ...
            if self.failure_count >= self.failure_threshold:
                ...
                logger.warning(
                    "LLM circuit breaker tripped", failure_count=self.failure_count
                )
