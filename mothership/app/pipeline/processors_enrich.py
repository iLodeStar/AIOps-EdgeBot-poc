...
class SeverityMapProcessor(Processor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized SeverityMap processor", mappings=len(self.severity_mapping)
        )
...
class ServiceFromPathProcessor(Processor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized ServiceFromPath processor",
    ...
    async def process(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for field in path_fields:
            if field in processed_event and processed_event[field]:
                service_name = self._extract_service_from_path(
                    str(processed_event[field])
                )
                if service_name:
                    break
        ...
        if not service_name:
            ...
            for field in hostname_fields:
                if field in processed_event and processed_event[field]:
                    service_name = self._extract_service_from_hostname(
                        str(processed_event[field])
                    )
                    if service_name:
                        break
...
class GeoHintProcessor(Processor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        logger.info(
            f"Initialized GeoHint processor",
...
class SiteEnvTagsProcessor(Processor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.site_patterns = config.get(
            "site_patterns",
            [
                (
                    r"\.([a-z]{2,3})\.(prod|production)\.",
                    {"env": "production", "site": r"\1"},
                ),
                (
                    r"\.([a-z]{2,3})\.(dev|development)\.",
                    {"env": "development", "site": r"\1"},
                ),
                (r"\.([a-z]{2,3})\.(test|testing)\.", {"env": "test", "site": r"\1"}),
                (
                    r"\.([a-z]{2,3})\.(stage|staging)\.",
                    {"env": "staging", "site": r"\1"},
                ),
            ],
        )
        ...
        logger.info(
            f"Initialized SiteEnvTags processor", patterns=len(self.compiled_patterns)
        )
    ...
    async def process(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if hostname:
            ...
            if site or env:
                ...
                logger.debug(
                    "Added site/env tags", hostname=hostname, site=site, env=env
                )

        # Add defaults if not set
        if (
            self.default_site
            and "tags" in processed_event
            and "site" not in processed_event["tags"]
        ):
        ...
        if (
            self.default_env
            and "tags" in processed_event
            and "environment" not in processed_event["tags"]
        ):
...
class TimestampNormalizer(Processor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.timestamp_fields = config.get(
            "timestamp_fields", ["timestamp", "time", "@timestamp"]
        )
        self.default_timezone = config.get("default_timezone", "UTC")

        logger.info(
            f"Initialized TimestampNormalizer processor", fields=self.timestamp_fields
        )
    ...
    async def process(self, event: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not any(field in processed_event for field in self.timestamp_fields):
            processed_event["timestamp"] = datetime.now(timezone.utc).strftime(
                "%Y-%m-%dT%H:%M:%SZ"
            )
