...
class WeatherPoller:
    ...
    async def _send_error_message(self, error: str):
        ...
        try:
            ...
            logger.error(
                "Error sending weather error message", error=str(callback_error)
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "interval": self.config.get("interval", 3600),
            "next_poll": (
                self.last_poll + self.config.get("interval", 3600)
                if self.last_poll > 0
                else None
            ),
        }
