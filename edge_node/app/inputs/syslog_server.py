...
FACILITIES = {
    0: "kernel", 1: "user", 2: "mail", 3: "daemon", 4: "security", 5: "syslogd",
...
class SyslogParser:
    ...
    def _parse_rfc3164(match: re.Match, message_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        pri = int(groups['pri'])
        facility, severity, facility_name, severity_name = SyslogParser.parse_priority(pri)
    ...
    def _parse_rfc5424(match: re.Match, message_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        message_data.update({
            ...
            "structured_data": groups['structured_data'] if groups['structured_data'] != '-' else None,
            "message": groups['message']
        })
    ...
    def datagram_received(self, data: bytes, addr: Tuple[str, int]):
        ...
        try:
            if len(data) > self.max_message_size:
                logger.warning("Received oversized UDP message", 
                             size=len(data), max_size=self.max_message_size, 
                             source=addr[0])
                return
            ...
            logger.error("Error processing UDP syslog message", 
                        error=str(e), source=addr[0])
...
class SyslogTCPHandler:
    ...
    def __init__(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter,
                 message_callback: Callable, max_message_size: int = 8192):
        self.reader = reader
        self.writer = writer
        self.message_callback = message_callback
        self.max_message_size = max_message_size
    ...
    async def handle_connection(self):
        ...
        try:
            while True:
                ...
                if len(data) > self.max_message_size:
                    logger.warning("Received oversized TCP message",
                                 size=len(data), max_size=self.max_message_size,
                ...
                if message:
                    parsed_message = SyslogParser.parse_message(message, self.client_addr)
            ...
            logger.error("Error in TCP syslog connection", 
                        error=str(e), client=self.client_addr[0])
        finally:
            self.writer.close()
...
class SyslogServer:
    ...
    async def start(self):
        ...
        logger.info("Syslog server started",
                   udp_port=udp_port, tcp_port=tcp_port, bind_address=bind_address)
    
    async def _handle_tcp_client(self, reader: asyncio.StreamReader, 
                                writer: asyncio.StreamWriter):
        """Handle new TCP client connection."""
        max_message_size = self.config.get('max_message_size', 8192)
        handler = SyslogTCPHandler(reader, writer, self.message_callback, max_message_size)
...
def create_syslog_server(config: Dict[str, Any], message_callback: Callable) -> SyslogServer:
