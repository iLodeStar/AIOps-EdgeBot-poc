    ...
    def __init__(self, config: Dict[str, Any], message_callback: Callable):
        ...
        for target_config in config.get('targets', []):
            try:
                ...
                logger.info("Added SNMP target", 
                           host=target.host, interval=target.interval, 
                           oids=len(target.oids))
            except Exception as e:
                logger.error("Failed to create SNMP target", 
                           config=target_config, error=str(e))
        ...
        try:
            ...
            for oid in target.oids:
                try:
                    ...
                    logger.warning("Error polling OID",
                                 host=target.host, oid=oid, error=str(e))
            ...
            if results:
                ...
                logger.debug("SNMP poll successful",
                           host=target.host, oids_polled=len(results))
            else:
                target.mark_failure()
            ...
            target.mark_failure()
            logger.error("Error polling SNMP target",
                        host=target.host, error=str(e))
            ...
            try:
                ...
                logger.error("Error sending SNMP error message",
...
def create_snmp_poller(config: Dict[str, Any], message_callback: Callable) -> SNMPPoller:
