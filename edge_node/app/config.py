...
class ConfigManager:
    ...
    def load_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                "Configuration loaded successfully", config_file=str(self.config_path)
            )
            return self._config
    ...
    def _apply_env_overrides(self):
        ...
        if os.getenv("EDGEBOT_MOTHERSHIP_URL"):
            self._config.setdefault("output", {}).setdefault("mothership", {})[
                "url"
            ] = os.getenv("EDGEBOT_MOTHERSHIP_URL")
        if os.getenv("EDGEBOT_AUTH_TOKEN"):
            self._config.setdefault("output", {}).setdefault("mothership", {})[
                "auth_token"
            ] = os.getenv("EDGEBOT_AUTH_TOKEN")

        # Input configuration
        if os.getenv("EDGEBOT_SYSLOG_UDP_PORT"):
            self._config.setdefault("inputs", {}).setdefault("syslog", {})[
                "udp_port"
            ] = int(os.getenv("EDGEBOT_SYSLOG_UDP_PORT"))
        if os.getenv("EDGEBOT_SYSLOG_TCP_PORT"):
            self._config.setdefault("inputs", {}).setdefault("syslog", {})[
                "tcp_port"
            ] = int(os.getenv("EDGEBOT_SYSLOG_TCP_PORT"))

        # Weather configuration
        if os.getenv("EDGEBOT_WEATHER_LAT"):
            self._config.setdefault("inputs", {}).setdefault("weather", {})[
                "latitude"
            ] = float(os.getenv("EDGEBOT_WEATHER_LAT"))
        if os.getenv("EDGEBOT_WEATHER_LON"):
            self._config.setdefault("inputs", {}).setdefault("weather", {})[
                "longitude"
            ] = float(os.getenv("EDGEBOT_WEATHER_LON"))
        if os.getenv("EDGEBOT_WEATHER_CITY"):
            self._config.setdefault("inputs", {}).setdefault("weather", {})["city"] = (
                os.getenv("EDGEBOT_WEATHER_CITY")
            )

        # NMEA input configuration
        if os.getenv("EDGEBOT_NMEA_ENABLED"):
            self._config.setdefault("inputs", {}).setdefault("nmea", {})["enabled"] = (
                os.getenv("EDGEBOT_NMEA_ENABLED").lower() in ("true", "1", "yes", "on")
            )
        if os.getenv("EDGEBOT_NMEA_UDP_PORT"):
            self._config.setdefault("inputs", {}).setdefault("nmea", {})["udp_port"] = (
                int(os.getenv("EDGEBOT_NMEA_UDP_PORT"))
            )

        # Persistent queue configuration (new)
        if os.getenv("QUEUE_ENABLED"):
            self._config.setdefault("queue", {})["enabled"] = os.getenv(
                "QUEUE_ENABLED"
            ).lower() in ("true", "1", "yes", "on")
        if os.getenv("QUEUE_DIR"):
            self._config.setdefault("queue", {})["dir"] = os.getenv("QUEUE_DIR")
        if os.getenv("QUEUE_MAX_BYTES"):
            self._config.setdefault("queue", {})["max_bytes"] = int(
                os.getenv("QUEUE_MAX_BYTES")
            )
        if os.getenv("QUEUE_FLUSH_INTERVAL_MS"):
            self._config.setdefault("queue", {})["flush_interval_ms"] = int(
        ...
        if os.getenv("FLUSH_BANDWIDTH_BYTES_PER_SEC"):
            self._config.setdefault("queue", {})["flush_bandwidth_bytes_per_sec"] = int(
