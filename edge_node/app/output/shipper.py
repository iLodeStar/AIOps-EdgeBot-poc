...
def build_sanitized_envelope(
    batch_messages: List[Dict[str, Any]], is_retry: bool = False
) -> str:
    ...
    def put(self, message: Dict[str, Any]) -> bool:
        ...
        if self._spool:
            ...
            if len(self._queue) >= self.max_size:
                self._dropped_messages += 1
                logger.warning(
                    "Message buffer full, dropping message",
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        if self._spool:
            ...
            return {
                ...
                "dropped_messages": self._dropped_messages,
                "utilization": (
                    len(self._queue) / self.max_size if self.max_size > 0 else 0
                ),
                "disk_buffer": False,
            }
...
class RetryManager:
    ...
    def add_failed_batch(
        ...
        retry_delay = (
            self._get_retry_after_delay(response_headers) if response_headers else None
        )
            ...
            if current_time >= next_retry_time:
                if attempt_count <= self.max_retries:
                    ...
                    remaining_batches.append(
                        (batch, attempt_count + 1, current_time + base_backoff)
                    )

                    logger.debug(
                        "Batch ready for retry",
                        messages=len(batch),
                    ...
                    logger.warning(
                        "Batch exceeded max retries, dropping",
                        messages=len(batch),
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_messages_in_retry": sum(
                len(batch) for batch, _, _ in self._retry_batches
            ),
        }
...
class IdempotencyManager:
    ...
    async def _clean_expired_keys(self, current_time: float):
        ...
        expired_keys = [
            key for key, timestamp in self._sent_keys.items() if timestamp < cutoff_time
        ]
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {"cached_keys": len(self._sent_keys), "window_sec": self.window_sec}
...
class DataShipper:
    ...
    async def _ship_loop(self):
        ...
        while self.running:
            try:
                ...
                should_send = self.buffer.size() >= batch_size or (
                    self.buffer.size() > 0
    ...
    async def _send_batch(self, batch: List[Dict[str, Any]], is_retry: bool = False):
        ...
        try:
            ...
            if parsed_url.scheme == "file":
                ...
                # HTTP-based output
                success, response_headers = await self._send_http_batch(
                    json_data, batch
                )
    ...
    async def _write_to_file(self, json_data: str, base_path: str):
        ...
        logger.info(
            "Payload written to files",
            gzip_file=gzip_path,
            json_file=json_path,
            size_bytes=len(json_data),
        )

    async def _send_http_batch(
        self, json_data: str, batch: List[Dict[str, Any]]
        ...
        try:
            ...
            # Send request
            logger.debug(
                "Sending batch to mothership", messages=len(batch), bytes=len(data)
            )

            response = await self.client.post(
                self.config["url"], content=data, headers=headers
            )

            response.raise_for_status()

            logger.debug(
                "HTTP batch sent successfully",
                messages=len(batch),
            ...
            if e.response:
                ...
                if status_code == 429:
                    ...
                    logger.warning(
                        "Rate limited by server, will retry with backoff",
                        status=status_code,
                        messages=len(batch),
                        retry_after=response_headers.get(
                            "Retry-After", "not specified"
                        ),
                    )
                    ...
                    logger.error(
                        "Client error sending batch, not retrying",
                        status=status_code,
                        messages=len(batch),
                    )
                    ...
                    logger.warning(
                        "Server error sending batch, will retry",
                        status=status_code,
                        messages=len(batch),
                    )
                    raise  # Re-raise to trigger retry
            ...
            logger.warning(
                "Network error sending batch, will retry",
                error=str(e),
                messages=len(batch),
            )
            raise  # Re-raise to trigger retry
