...
class BandwidthLimiter:
    ...
    def _replenish_tokens(self):
        ...
        self._tokens = min(
            self.burst_bytes, self._tokens + elapsed * self.bytes_per_sec
        )
...
class PersistentQueue:
    ...
    def enqueue(self, message: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if current_size + message_size > self.max_bytes:
                logger.warning(
                    "Queue approaching capacity, applying backpressure",
    ...
    async def _flush_ready_messages(self):
        ...
        if not self.bandwidth_limiter.can_send(batch_bytes):
            ...
            if wait_time > 0:
                logger.debug(
                    "Bandwidth limit reached, waiting",
                    wait_time=wait_time,
        ...
        if success_count > 0:
            successful_messages = [
                msg for msg in messages if msg not in failed_messages
            ]
