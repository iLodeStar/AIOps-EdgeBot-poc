...
async def ship_to_file(buffer_path: str, output_dir: str, batch_size: int = 100):
    ...
    try:
        ...
        if wait_cycle >= max_wait_cycles:
            print(
                "Warning: Maximum wait time reached, some messages may not have been shipped"
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", required=True, help="Output directory for payload files"
    )
    parser.add_argument(
        "--batch-size", type=int, default=100, help="Batch size for shipping"
    )
    parser.add_argument(
        "--create-output-dir",
        action="store_true",
    ...
    if not os.path.exists(args.output_dir):
        if args.create_output_dir:
            ...
            print(
                f"Error: Output directory '{args.output_dir}' not found",
                file=sys.stderr,
            )
