...
def normalize_syslog_event(
    event: Dict[str, Any], tz: Optional[str] = None, map_severity: bool = False
) -> Dict[str, Any]:
...
def normalize_snmp_metric(
    event: Dict[str, Any], tz: Optional[str] = None, percent_as_ratio: bool = False
) -> Dict[str, Any]:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Import JSONL events into EdgeBot buffer"
    )
    parser.add_argument(
        "jsonl_file",
        nargs="?",
        help="Path to the JSONL file to import (use - for stdin)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--record-type",
        choices=["syslog_event", "snmp_metric"],
        default="syslog_event",
        ...
        for line_num, line in enumerate(input_file, 1):
            ...
            try:
                ...
                if args.record_type == "snmp_metric":
                    message = normalize_snmp_metric(
                        event, args.tz, args.percent_as_ratio
                    )
                else:
