...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description='Dump EdgeBot SQLite spool database contents')
    ...
    parser.add_argument('--messages', type=int, default=0,
                       help='Show sample messages (specify limit)')
    parser.add_argument('--status', choices=['pending', 'completed', 'failed'],
                       help='Filter messages by status')
    
    # Cleanup options
    parser.add_argument('--cleanup', type=int, metavar='SECONDS',
    ...
    parser.add_argument('--all', action='store_true', 
                       help='Show schema, stats, and 10 sample messages')
    ...
    try:
        ...
        if not any([args.schema, args.stats, args.messages > 0, args.cleanup is not None, args.all]):
            dump_stats(args.db_path)
