...
def parse_csv_row(row: Dict[str, str], tz: Optional[str] = None) -> Dict[str, Any]:
    ...
    for key, value in row.items():
        if key not in [
            ...
            "city",
            "location",
        ] and key not in [k for keys in field_mappings.values() for k in keys]:
            try:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Import weather CSV data into EdgeBot buffer"
    )
    parser.add_argument(
        "csv_file", nargs="?", help="Path to the CSV file to import (use - for stdin)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--max-size", type=int, default=10000, help="Maximum buffer size"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Parse CSV but do not insert into buffer"
    )
    parser.add_argument(
        "--max-lines", type=int, help="Maximum number of lines to process"
    )
    parser.add_argument(
        "--tz", help="IANA timezone for naive timestamps (e.g., Asia/Kolkata)"
    )

    args = parser.parse_args()

    # Handle stdin vs file input
    if args.stdin or args.csv_file == "-":
        if args.csv_file and args.csv_file != "-" and args.stdin:
        ...
        if not os.path.exists(args.csv_file):
            ...
            sys.exit(1)
        input_file = open(args.csv_file, "r", encoding="utf-8")
    ...
    if args.use_memory:
        ...
        buffer = MessageBuffer(
            max_size=args.max_size, disk_buffer=True, disk_path=args.buffer_path
        )
    ...
    try:
        # Detect delimiter if reading from file
