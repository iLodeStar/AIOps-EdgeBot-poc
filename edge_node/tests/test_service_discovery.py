...
class TestServiceDiscovery(unittest.IsolatedAsyncioTestCase):
    async def asyncSetUp(self):
        if not DISCOVERY_AVAILABLE:
            self.skipTest("ServiceDiscovery/FileTailer not available (branch not merged yet)")
        self.messages = []
        ...
        self.discovery = ServiceDiscovery(
            {"enabled": True, "interval": 1, "auto_tail_logs": True, "extra_logs": [self.tmp_path]},
            self.cb,
            tailer=self.tailer,
        )
    ...
    async def test_discover_once(self, mock_ss):
        ...
        self.assertTrue(any(m.get("type") == "host_service_inventory" for m in self.messages))
