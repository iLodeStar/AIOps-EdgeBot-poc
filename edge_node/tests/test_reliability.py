...
class TestEnhancedRetryManager:
    ...
    def test_add_failed_batch_with_retry_after(self):
        ...
        assert (
            next_retry_time >= initial_time + 29.5
    ...
    def test_exponential_backoff_progression(self):
        ...
        rm = RetryManager(
            max_retries=5, initial_backoff_ms=100, max_backoff_ms=5000, jitter_factor=0
        )
        ...
        rm._retry_batches[0] = (
            rm._retry_batches[0][0],
            rm._retry_batches[0][1],
            time.time() - 1,
        )
    ...
    def test_max_retries_exceeded(self):
        ...
        rm = RetryManager(
            max_retries=2, initial_backoff_ms=100, max_backoff_ms=1000, jitter_factor=0
        )
    ...
    async def test_full_reliability_stack(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch("httpx.AsyncClient") as mock_client_class:
                ...
                # Add test message
                buffer.put(
                    {"message": "test message", "timestamp": "2023-01-01T00:00:00Z"}
                )
